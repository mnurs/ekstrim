            ...
            <div class="row align-items-center">
                ...
                <div class="col-md-6">
                    <div class="register-form">
                        <h2 class="form-title">{{ __('Daftar Ekstrim') }}</h2>
                        <form method="POST" action="{{ route('user.create') }}">
                            ...
                            <div class="form-group">
                                <input type="email" class="form-control" id="email" name="email" value="{{ old('email') }}" placeholder="{{ __('Masukkan Email') }}" />
                                <i class="fas fa-envelope form-icon"></i>
                                <span class="text-danger">{{ __($errors->first('email')) }}</span>
                            </div>
                            <div class="form-group">
                                <input type="password" class="form-control" id="password" name="password" placeholder="{{ __('Masukkan Password') }}" />
                            ...
                            <div class="form-group form-check">
                                ...
                                <label class="form-check-label" for="Agree">{{ __('Saya setuju dengan syarat & ketentuan.') }}</label>
                                <br>
                                <span class="text-danger">{{ __($errors->first('Setuju')) }}</span>
                            </div>
                            <div class="form-bottom">
                                <button type="submit" class="primary-btn">{{ __('Daftar Akun') }}</button>
                                <p class="form-bottom-text">{{ __('Sudah punya Akun ? Silahkan') }} <a href="{{route('signin')}}">{{ __('Masuk') }}</a></p>
                            </div>
                        </form>
                    </div>
                </div>
