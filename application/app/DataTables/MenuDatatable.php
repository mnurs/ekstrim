    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('slug', function($query) {
                return '<code>/'. $query->slug .'</code>';
            })

            ->addColumn(
                'action',
                '<a href="{{route("menu.edit", $id)}}"><i class="ik ik-edit"></i></a>
                <a class="delete" href="{{route("menu.delete", $id)}}"><i class="ik ik-trash-2"></i></a> '
            )->rawColumns(['image', 'action', 'status', 'slug']);
    }
